...
const MakeCourse: React.FC = () => {
  ...
  const [problemList, setProblemList] = useState<CourseItem[]>([]);
  ...
  const makeQuestion = () => {
    const fileInput = document.getElementById("problemPdf") as HTMLInputElement;
    ...
    const problemPdf = fileInput.files[0];
    ...
    axios.post(`${config.SERVER_URL}course/make-problem`, formData, {
      headers: {
        'Content-Type': 'multipart/form-data'
      }
    }).then(res => {
      toast.info(`Completed upload and made problems successfully in ${res.data.language} language`);
      readQuiz();
    }).catch(error => {
  ...
  const handleDelete = (event: React.MouseEvent, item: CourseItem) => {
    const requestData = {
      _id: item._id
    }
                  ...
                  <div className="w-full sm:w-1/3">
                    <div className="relative z-20 bg-transparent dark:bg-form-input">
                      <select
                        ...
                        className={`relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary ${isLanguageSelected ? 'text-black dark:text-white' : ''}`}
                      >
                        <option value="auto" className="text-body dark:text-bodydark">
                        ...
                        <option value="fr" className="text-body dark:text-bodydark">
                          French
                        </option>
                        <option value="de" className="text-body dark:text-bodydark">
                          German
                        </option>
                        <option value="it" className="text-body dark:text-bodydark">
                          Italian
                        </option>
                        <option value="pt" className="text-body dark:text-bodydark">
                          Portuguese
                        </option>
                      </select>
                      <span className="absolute top-1/2 right-4 z-30 -translate-y-1/2">
